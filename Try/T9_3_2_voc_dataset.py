...
from Try.T9_3_5_sampling import sampleEzDetect
...
class vocDataset(data.Dataset):
    def __init__(self, config, isTraining=True):
        ...
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                         std=[0.229, 0.224, 0.225])  # 使用均值和方差对图片的RGB值分别进行归一化（也可以使用其他方法，只是这种方法相对来说比较简单）
...
print(vocClassID)
allTrainingData = []  # 第167行，该行后面的代码为从VOC2007中读取数据，其会在调用voc_dataset.py文件时立即执行
allTestingData = []
allFloder = ["../ml/VOCdevkit/VOC2007"]  # 我们将从VOC网站上下载的数据放到本地，这里只使用VOC2007做实验
