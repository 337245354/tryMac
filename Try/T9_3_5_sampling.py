...
from Try.T9_3_5_bbox import bboxIOU
...
def buildPredBoxes(config):
    ...
    for i in range(len(config.mboxes)):
        ...
        for y in range(hei):
            for x in range(wid):
                xc = (x + 0.5) / wid  # x y位置取每个feature map像素的中心点来计算
                yc = (y + 0.5) / hei
                '''
...
def sampleEzDetect(config, bboxes):  # 在voc_dataset.py的vocDataset类中用到了samp-leEzDetect函数
